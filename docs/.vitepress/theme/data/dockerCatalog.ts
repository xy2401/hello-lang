...
export const dockerCatalog: DockerCatalogEntry[] = [
  ...
  { id: 'groovy', name: 'Groovy', mode: 'custom-image', status: 'documented', images: [
    { role: 'builder', tag: 'eclipse-temurin:25-jdk + groovy-5.1.0', source: 'custom-official-base' },
    { role: 'runtime', tag: 'eclipse-temurin:25-jre', source: 'official' },
  ], toolRoots: ['/opt/groovy/bin', '/opt/java/openjdk/bin', '/usr/local/bin', '/usr/bin'], keyTools: ['groovy', 'groovyc', 'groovysh', 'grape', 'java'], buildCommand: 'groovyc -d /out Hello.groovy', runCommand: 'groovy Hello.groovy', note: '在 Temurin 官方 JDK 上安装 Apache Groovy 固定发行包；证据尚待 Docker 工作流采集。' },
  { id: 'scala', name: 'Scala', mode: 'custom-image', status: 'documented', images: [
    { role: 'builder', tag: 'eclipse-temurin:25-jdk + scala-3.3.8', source: 'custom-official-base' },
    { role: 'runtime', tag: 'eclipse-temurin:25-jre', source: 'official' },
  ], toolRoots: ['/opt/scala/bin', '/opt/java/openjdk/bin', '/usr/local/bin', '/usr/bin'], keyTools: ['scala', 'scalac', 'scaladoc', 'java'], buildCommand: 'scalac -d /out Hello.scala', runCommand: 'scala Hello', note: 'Scala 3.3 LTS 固定发行包与 Temurin 官方 JDK 组合；证据尚待 Docker 工作流采集。' },
  { id: 'clojure', name: 'Clojure', mode: 'custom-image', status: 'documented', images: [
    { role: 'builder', tag: 'eclipse-temurin:25-jdk + clojure-tools-1.12.5.1664', source: 'custom-official-base' },
    { role: 'runtime', tag: 'eclipse-temurin:25-jre', source: 'official' },
  ], toolRoots: ['/usr/local/bin', '/opt/java/openjdk/bin', '/usr/bin'], keyTools: ['clojure', 'clj', 'java'], buildCommand: "clojure -M -e '(compile (quote hello.core))'", runCommand: 'clojure -M -m hello.core', note: '官方 Clojure CLI 安装在 Temurin JDK 上，并在构建阶段预取基础依赖；证据尚待 Docker 工作流采集。' },
  ...
  { id: 'lisp', name: 'Lisp 家族', mode: 'custom-image', status: 'documented', images: [
    { role: 'runtime', tag: 'clfoundation/sbcl:2.6.8', digestKey: 'CLFOUNDATION_SBCL_2_6_8_IMAGE', source: 'vendor' },
    { role: 'runtime', tag: 'hello-lang-guile:3.0.11', source: 'custom-official-base' },
    { role: 'runtime', tag: 'hello-lang-clojure:1.12.5', source: 'custom-official-base' },
    { role: 'runtime', tag: 'hello-lang-racket:9.3', source: 'custom-official-base' },
  ], toolRoots: ['/usr/local/bin', '/opt/guile/bin', '/usr/local/lib/clojure', '/opt/racket/bin'], keyTools: ['sbcl', 'guile', 'clojure', 'racket', 'raco'], buildCommand: 'docker build Dockerfile.guile / Dockerfile.clojure / Dockerfile.racket', runCommand: 'sbcl --script | guile -s | clojure -M | racket', note: '聚合产品同时记录四个方言运行时；Common Lisp 使用社区基金会镜像，其余自定义镜像从官方发布物或官方基础镜像构建。' },
  { id: 'lua', name: 'Lua', mode: 'custom-image', status: 'documented', images: [
    { role: 'builder', tag: 'gcc:14', digestKey: 'GCC_14_IMAGE', source: 'official' },
    { role: 'runtime', tag: 'hello-lang-lua:5.5.1', source: 'custom-official-base' },
  ], toolRoots: ['/opt/lua/bin', '/usr/local/bin', '/usr/bin'], keyTools: ['lua', 'luac'], buildCommand: 'make && make INSTALL_TOP=/opt/lua install', runCommand: 'lua main.lua', note: 'Lua 5.5.1 从 lua.org 官方源码和固定 SHA-256 构建，不依赖第三方成品镜像。' },
  { id: 'html', name: 'HTML', mode: 'custom-image', status: 'documented', images: [{ role: 'builder', tag: 'node:24-bookworm-slim + pug@3.0.4 + html-validate@11.10.0', source: 'custom-official-base' }, { role: 'runtime', tag: 'nginx:1.28-alpine', digestKey: 'NGINX_1_28_ALPINE_IMAGE', source: 'official' }], toolRoots: ['/usr/local/bin', '/usr/bin'], keyTools: ['node', 'npm', 'pug-build', 'html-validate', 'nginx'], buildCommand: 'pug-build index.pug dist/index.html && html-validate dist/index.html', runCommand: 'nginx -g "daemon off;" + HTTP GET /' },
  { id: 'css', name: 'CSS', mode: 'custom-image', status: 'documented', images: [{ role: 'builder', tag: 'node:24-bookworm-slim + sass@1.103.1 + postcss@8.5.26 + stylelint@17.14.1', source: 'custom-official-base' }, { role: 'runtime', tag: 'nginx:1.28-alpine', digestKey: 'NGINX_1_28_ALPINE_IMAGE', source: 'official' }], toolRoots: ['/usr/local/bin', '/usr/bin'], keyTools: ['node', 'npm', 'sass', 'postcss', 'autoprefixer', 'stylelint', 'nginx'], buildCommand: 'sass style.scss style.css && postcss style.css --use autoprefixer -o dist/style.css && stylelint dist/style.css', runCommand: 'nginx -g "daemon off;" + HTTP GET /dist/style.css' },
]
